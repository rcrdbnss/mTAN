...
class PhysioNet_(PhysioNet):
    ...
    def download(self):
        ...
        self._download_outcomes()
        self._download_and_extract_main_datasets()
        self._process_extracted_files()

    def _download_outcomes(self):
        for url in self.outcome_urls:
            filename = url.rpartition('/')[2]
            ...
            with open(txtfile) as f:
                lines = f.readlines()
                outcomes = {
                    l.split(',')[0]: torch.Tensor(np.array(l.rstrip().split(',')[1:]).astype(float)).to(self.device) for
                    l in lines[1:]}

            torch.save(outcomes, os.path.join(self.processed_folder, filename.split('.')[0] + '.pt'))

    def _download_and_extract_main_datasets(self):
        for url in self.urls:
            filename = url.rpartition('/')[2]
            if filename.count('?') == 1:
                filename = filename.split('?')[0]
            self._download_url(url, filename)
            self._extract_tar_gz(filename)
    ...
    def _process_extracted_files(self):
        for url in self.urls:
            ...
            for txtfile in os.listdir(dirname)[0:5]:
                record_id = txtfile.split('.')[0]
                patient_data = self._process_patient_file(os.path.join(dirname, txtfile), record_id)
                if patient_data:
                    patients.append(patient_data)

            torch.save(patients, os.path.join(self.processed_folder,
                                              filename.split('.')[0] + "_" + str(self.quantization) + '.pt'))

        print('Done!')

    def _process_patient_file(self, filepath, record_id):
        with open(filepath) as f:
            ...
            labels = None
            outcomes = torch.load(os.path.join(self.processed_folder, self.label_file), map_location=self.device)
            if record_id in outcomes:
                labels = outcomes[record_id][4]

            return (record_id, tt, vals, mask, labels)
